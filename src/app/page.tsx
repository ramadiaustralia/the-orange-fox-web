...
const iconMap: Record<string, React.ComponentType<{className?: string}>> = {
  ...
  return (
    <>
      {/* Hero Section */}
      <section className="relative bg-gradient-to-br from-dark via-dark-soft to-charcoal px-6 pt-12 pb-10 md:pt-16 md:pb-12 text-center overflow-hidden">
        ...
        <div className="absolute w-[300px] h-[300px] rounded-full -top-20 -left-20 pointer-events-none opacity-40" style={{ background: 'radial-gradient(circle,rgba(212,105,42,0.08) 0%,transparent 70%)', animation: 'orbFloat 30s ease-in-out infinite reverse' }} />
        {/* Tertiary orb bottom-right */}
        <div className="absolute w-[250px] h-[250px] rounded-full -bottom-16 -right-16 pointer-events-none opacity-30" style={{ background: 'radial-gradient(circle,rgba(212,105,42,0.1) 0%,transparent 70%)', animation: 'orbFloat 35s ease-in-out infinite' }} />
        ...
        {[...Array(6)].map((_, i) => (
          <div key={i} className="absolute w-1 h-1 rounded-full bg-orange/30 pointer-events-none" style={{
            ...
            animation: `twinkle ${4 + i * 1.2}s ease-in-out infinite ${i * 0.8}s, heroParticleFloat ${14 + i * 5}s ease-in-out infinite ${i * 2}s`
          }} />
        ))}
        
        {/* Diagonal accent lines */}
        <div className="absolute top-0 right-[15%] w-px h-32 pointer-events-none opacity-10" style={{ background: 'linear-gradient(to bottom, transparent, rgba(212,105,42,0.5), transparent)', animation: 'heroLineSlide 10s ease-in-out infinite' }} />
        <div className="absolute bottom-0 left-[20%] w-px h-24 pointer-events-none opacity-10" style={{ background: 'linear-gradient(to top, transparent, rgba(212,105,42,0.4), transparent)', animation: 'heroLineSlide 12s ease-in-out infinite 3s' }} />
        
        {/* Corner accents */}
        <div className="absolute top-8 left-8 w-16 h-16 pointer-events-none opacity-10 border-l border-t border-orange/30" style={{ animation: 'heroCornerPulse 7s ease-in-out infinite' }} />
        <div className="absolute bottom-8 right-8 w-16 h-16 pointer-events-none opacity-10 border-r border-b border-orange/30" style={{ animation: 'heroCornerPulse 7s ease-in-out infinite 3.5s' }} />
        ...
        <div className="relative z-10 max-w-2xl mx-auto">
          ...
          <div className="relative w-20 h-px mx-auto my-5">
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-orange to-transparent" style={{ animation: 'heroDividerGlow 5s ease-in-out infinite' }} />
          </div>

          {/* Title with stagger animation */}
          <h1 className="text-white text-[clamp(1.8rem,4.5vw,2.8rem)] font-bold leading-tight mb-3" style={{ fontFamily: 'var(--font-heading)', animation: 'heroSlideUp 1.4s ease-out 0.4s both' }}>
            {t('hero_title')}
          </h1>
          <p className="text-white/40 text-[0.85rem] max-w-md mx-auto mb-8 leading-relaxed" style={{ fontFamily: 'var(--font-body)', animation: 'heroSlideUp 1.4s ease-out 0.8s both' }}>
            {t('hero_desc')}
          </p>

          {/* CTAs with animation */}
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4" style={{ animation: 'heroSlideUp 1.4s ease-out 1.2s both' }}>
            <Link
              href="/contact"
