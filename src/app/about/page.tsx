...
export default function AboutPage() {
  ...
  const values = [
    { icon: '💎', titleKey: 'value_quality' as const, descKey: 'value_quality_desc' as const },
    { icon: '🔍', titleKey: 'value_transparency' as const, descKey: 'value_transparency_desc' as const },
    { icon: '🤝', titleKey: 'value_partnership' as const, descKey: 'value_partnership_desc' as const },
            ...
            {values.map((v, i) => (
              <ScrollReveal key={i} delay={i * 0.12}>
                <div className="bg-white border border-border-light rounded-2xl p-7 text-center transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_20px_60px_rgba(212,105,42,0.08)] h-full">
                  <div className="w-12 h-12 rounded-xl bg-orange/10 flex items-center justify-center text-xl mx-auto mb-4">{v.icon}</div>
